...
(function() {
    ...
    function formatMathAndCleanText(text) {
        ...
        // 1. Remove primeiro todos os caracteres invisiveis de controle da OpenAI (\uE000-\uF8FF)
        text = text.replace(/[\uE000-\uF8FF]/g, '');

        // 2. Extrai o LaTeX dos blocos de widgets genui (blocos $$ ... $$)
        ...
        // 3. Processa e limpa a sintaxe de entity recuperando notação matemática em linha e descartando números soltos
        text = text.replace(/entity\[([\s\S]*?)\]/gi, function(match, inner) {
            // Verifica se o conteúdo interno possui marcação matemática LaTeX
            ...
            // Descarta numerais isolados ("1", "2") e rótulos do sistema da OpenAI
            const parts = inner.split('","').map(p => p.replace(/^"|"$/g, '').trim());
            for (let part of parts) {
                if (part && !/^\d+$/.test(part) && !part.startsWith('scientific_concept') && !part.startsWith('relatividade')) {
                    return part;
                }
            }
        ...
        // 4. Limpeza de resíduos de tags e lixo do React
        ...
        // 5. Normaliza termos do tensor de Ricci/vetores caso venham sem os delimitadores de bloco
        text = text.replace(/R_?\{?\\?mu\\?nu\}?\s*u_?\^?\\?mu\s*u_?\^?\\?nu/gi, '$R_{\\mu\\nu} u^\\mu u^\\nu$');

        // 6. Padronizacao do LaTeX ($$ e $)
