...
// @version      2.2.0
// @description  Exporta o histórico do ChatGPT convertendo LaTeX para padrão Markdown ($$) e ajustando anexos de imagens.
...
(function() {
    ...
    function formatMathAndCleanText(text) {
        ...
        // Convert LaTeX Block delimiters \[ ... \] to $$ ... $$
        text = text.replace(/\\\[\s*([\s\S]*?)\s*\\\]/g, '\n$$\n$1\n$$\n');

        // Convert LaTeX Inline delimiters \( ... \) to $ ... $
        text = text.replace(/\\\(\s*([\s\S]*?)\s*\\\)/g, '$$1$');

        // Clean internal script trash
        ...
        parts.forEach(part => {
            if (typeof part === 'string') {
                ...
                if (part.content_type === 'image_asset_pointer') {
                    if (includeImages) {
                        // Substitui a URL do sediment por um nome de imagem local padrão
