...
// @version      2.3.0
// @description  Exporta o histórico do ChatGPT convertendo blocos MathJax para $$ e $ compatíveis com Obsidian e Typora.
...
(function() {
    ...
    function formatMathAndCleanText(text) {
        ...
        // Remove escapes indevidos de cifrão no texto retornado pela API
        text = text.replace(/\\\$ /g, '$').replace(/\\\$/g, '$');

        // Converte blocos LaTeX \[ ... \] para $$ ... $$ (compatível com Obsidian/MathJax)
        text = text.replace(/\\\[\s*([\s\S]*?)\s*\\\]/g, '\n$$\n$1\n$$\n');

        // Converte equações em linha \( ... \) para $ ... $
        text = text.replace(/\\\(\s*([\s\S]*?)\s*\\\)/g, '$$1$');

        // Limpeza de códigos e rotinas internas do React
    ...
    async function startExportProcess() {
        ...
        const includeImages = confirm("Deseja INCLUIR os marcadores de imagens/anexos no arquivo?\n\n- OK: Insere marcadores para imagens locais (ex: imagem_1.png).\n- Cancelar: REMOVE todas as imagens.");
